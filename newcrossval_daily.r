...
walk_summable_rel = function(M) {
  ...
  nifret = read.csv("niftyreturns4.csv") #can be changed to see for diff no. of stocks and samples
dim(nifret)
 dailyvar <- function(y) {
dmat = y
...
nmgl1 = Nelder_Mead(glasso1, 5e-10, 0, Inf)
glasso2 <- function(p){
 res_glasso = cv2(X, glasso_train, p)
 return(res_glasso) 
}
nmgl2 = Nelder_Mead(glasso2, 5e-10, 0, Inf)
...
nmcl1 = Nelder_Mead(clime1, 5e-10, 0, Inf)
clime2 <- function(p){
 res_clime = cv2(X, clime_train, p)
 return(res_clime) 
 }
 nmcl2 = Nelder_Mead(clime2, 5e-10, 0, Inf)
 #mb
...
nmmb1 = Nelder_Mead(mb1, 5e-10, 0, Inf)
mb2 <- function(p){
 res_mb = cv2(X, mb_train, p)
 return(res_mb) 
} 
nmmb2 = Nelder_Mead(mb2, 5e-10, 0, Inf)
#hybridmb
...
nmhmb1 = Nelder_Mead(hybrid.mb1, 5e-10, 0, Inf)
hybrid.mb2 <- function(p){
 res_hybrid.mb = cv2(X, hybrid.mb_train, p)
 return(res_hybrid.mb) 
 }
 nmhmb2 = Nelder_Mead(hybrid.mb2, 5e-10, 0, Inf)
 #greedyprune
greedy_params = list()
for (k in log_grid_discrete(3,24,6)) {#log_grid_discrete(3, 26, 6)) {
 ...
 nGlobalVar = 0
  ...
  nmgreedy1 <- Nelder_Mead(greedy1, 5e-10, 0, Inf)
 ...
 grpr1 = c(greedylist1[[glind1]][1], greedylist1[[glind1]][2])
  ...
  nmgreedy2 <- Nelder_Mead(greedy2, 5e-10, 0, Inf)
